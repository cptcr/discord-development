...
export async function loadSlashCommands(clientId: string, guildId: string, token: string) {
    ...
    for (const file of slashFiles) {
      ...
      try {
        ...
        if (cmd?.data) {
          ...
          success(true, `Loaded Slash Command: ${cmd.data.name}`);
        }        
      } catch (err) {
  ...
  try {
    if (guildId) {
      ...
      success(true, `Registered ${commandsData.length} guild slash commands (Guild: ${guildId}).`);
    } else {
      // Register as *Global* commands (can take up to an hour to sync)
      await rest.put(Routes.applicationCommands(clientId), {
        body: commandsData,
      });
      success(true, `Registered ${commandsData.length} global slash commands.`);
    }
  } catch (regErr) {
...
export function loadPrefixCommands() {
  warn(true, "Loading Prefix Commands...");
    ...
    for (const file of prefixFiles) {
      ...
      try {
        ...
        if (cmd && cmd.name) {
          ...
          success(true, `Loaded Prefix Command: ${cmd.name}`);
        }        
      } catch (err) {
